...
class Company(BaseModel):
    ...
    current_ratio: float = Field(
        default=0.0, title="Current Ratio"
    )  # Not included in one of the CSV rows
    debt_to_equity_ratio: float
    location: str
    ceo: str = Field(
        default="Unknown", title="CEO"
    )  # Not included in all of the CSV rows
    number_of_employees: int = Field(
        default=0, title="Number of Employees"
    )  # Not included in all of the CSV rows
